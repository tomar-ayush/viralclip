...
from app.core.security import encrypt_api_key, generate_key_fingerprint
...
@router.post("/create", response_model=UserResponse, status_code=status.HTTP_201_CREATED, summary="Create new platform user")
...
@router.post("/keys", response_model=UserAPIKeyResponse, status_code=status.HTTP_201_CREATED, summary="Securely encrypt and store BYOK third-party API key")
async def store_user_api_key(
    request: UserAPIKeyCreate,
    session: AsyncSession = Depends(get_async_session)
):
    """
    Encrypts a user's API Key using AES-256-GCM before writing to PostgreSQL DB.
    Also stores a key fingerprint for display.
    """
