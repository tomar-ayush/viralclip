...
def _get_master_key_bytes() -> bytes:
    """
    Derives/parses 32-byte key for AES-256-GCM from ENCRYPTION_MASTER_KEY config.
    """
    ...
    # If not valid 32-byte hex, hash key_str to get fixed 32 bytes
    return hashlib.sha256(key_str.encode("utf-8")).digest()


def encrypt_api_key(plain_api_key: str) -> str:
    """
    Encrypts a third-party API key using AES-256-GCM.
    ...
    # Generate 12-byte nonce (IV) recommended for GCM
    nonce = os.urandom(12)
    
    ciphertext = aesgcm.encrypt(nonce, plain_api_key.encode("utf-8"), associated_data=None)
    
    # Bundle IV + Ciphertext together and base64 encode
...
def decrypt_api_key(encrypted_payload_b64: str) -> str:
    ...
    Extracts 12-byte IV and decrypts the ciphertext.
    """
