...
from app.schemas.script import ScriptResponse, SceneSchema
...
class LLMService:
    async def generate_script(
        ...
        """
        Calls OpenAI Chat Completions API with structured JSON output using user's decrypted BYOK key.
        """
        system_prompt = (
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    print(f"[LLM Warning] OpenAI API returned {response.status_code}: {response.text}. Using fallback mock synthesis.")
        except Exception as e:
            print(f"[LLM Error] OpenAI request failed ({e}). Using fallback mock script.")

        # Robust Fallback Script Generation for mock testing or API key errors
        return self._generate_fallback_script(topic, tone, target_duration_seconds)

    def _generate_fallback_script(self, topic: str, tone: str, target_duration: int) -> ScriptResponse:
        scenes = [
            SceneSchema(
                scene_number=1,
            ...
            SceneSchema(
                scene_number=2,
                text=f"Here is why everyone is talking about it. First, it completely changes how we think about automation.",
